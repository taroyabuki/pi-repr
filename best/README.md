...
- `long double`: x87 80-bit では正規化後の先頭の `1` を省略せずに明示的に持ちます．C では `0xC.90FDAA22168C235p-2L` でも best を直接書けます．
...
| `long double` | `4*atanl(1)`<br>`3.1415926535897932385L`（小数第20位を四捨五入）<br>`3.14159265358979323846L`<br>`0xC.90FDAA22168C235p-2L`<br>`8717442233.L/2774848045` |
...
- FM-7 BASIC と FM-11 BASIC は F-BASIC にまとめています．少なくとも `best/fm7basic.bas` / `best/fm11basic.bas` の rerun では，どちらも `MATCH OK` / `ATN GT` を返しました．
