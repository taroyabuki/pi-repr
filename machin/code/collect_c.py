...
CHECK = Path(__file__).with_name("C-maclaurin.c")
OUTPUT = Path("/tmp/check-c-machin")
...
def parse_output(text: str, limit: int) -> list[Row]:
    ...
    for raw_line in text.splitlines():
        ...
        label = match.group(1)
        found = match.group(3)
        current[label] = (describe(found if found is not None else "none", limit), f"`{match.group(4)}`")
    if current_name is not None:
        rows.append(Row(current_name, current))
    return rows
