...
RUNNER_CONFIGS = (
    RunnerConfig(
        "6502 BASIC",
        (str(CLASSIC_BASIC / "6502.sh"), "--run", "--file", str(BAS_DIR / "6502-pow.bas")),
    ),
    RunnerConfig(
        "BASIC-80",
        (
    ...
    RunnerConfig(
        ...
        (
            ...
            "--run",
            "--file",
            str(BAS_DIR / "nbasic-pow.bas"),
        ),
        timeout=600,
    ),
    RunnerConfig(
        "N88-BASIC",
        (str(CLASSIC_BASIC / "n88basic.sh"), "--run", "--file", str(BAS_DIR / "n88basic-pow.bas")),
        timeout=180,
    ),
    RunnerConfig(
        "FM-7 F-BASIC",
        (str(CLASSIC_BASIC / "fm7basic.sh"), "--run", "--timeout", "120s", "--file", str(BAS_DIR / "fm7basic-pow.bas")),
        timeout=180,
    ),
    RunnerConfig(
        "FM-11 F-BASIC",
    ...
    RunnerConfig(
        "MSX-BASIC",
        (
            str(CLASSIC_BASIC / "msxbasic.sh"),
            "--run",
            "--file",
            str(BAS_DIR / "msxbasic-pow.bas"),
            "--timeout",
            "300",
        ),
        timeout=320,
    ...
    RunnerConfig(
        "GW-BASIC",
        (str(CLASSIC_BASIC / "gwbasic.sh"), "--run", "--file", str(BAS_DIR / "gwbasic-pow.bas")),
        timeout=30,
    ),
    RunnerConfig(
        "QBasic",
        (str(CLASSIC_BASIC / "qbasic.sh"), "--run", "--file", str(BAS_DIR / "qbasic-pow.bas")),
        timeout=30,
    ),
    RunnerConfig(
        "Grant BASIC",
        ("python3", str(Path(__file__).with_name("collect_grantsbasic_pow.py"))),
        timeout=180,
    ),
)
    ...
    while i < len(lines):
        ...
        if line == "BEST":
            ...
            if j >= len(lines):
                i += 1
                continue
            best, next_i = read_bytes_from(lines, j)
            if len(best) >= 4:
                result["BEST"] = best
            i = next_i
            continue
        ...
        if match:
            label = match.group(1)
            j = _next_nonempty(lines, i + 1)
            current, next_i = read_bytes_from(lines, j)
            k = _next_nonempty(lines, next_i)
            if k >= len(lines):
                i += 1
                continue
            diff = lines[k].strip()
...
def row_from_output(name: str, parsed: dict[str, tuple[str, list[str], str] | list[str]], mode: str) -> tuple[str, str, str, str, str, str, str]:
    best = parsed["BEST"]
...
def main() -> int:
    outputs: list[tuple[str, dict[str, tuple[str, list[str], str] | list[str]]]] = []
    for config in RUNNER_CONFIGS:
        try:
            outputs.append((config.name, parse_output(run_probe(config))))
    ...
    for mode in ("TERM", "SPLIT"):
        ...
        print("| --- | --- | --- | --- | --- | --- | --- |")
        for name, parsed in outputs:
            print("| " + " | ".join(row_from_output(name, parsed, mode)) + " |")
        if mode != "SPLIT":
            print()
    return 0
